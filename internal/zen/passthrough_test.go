...
func TestForwardMessages_DefaultAnthropicVersion(t *testing.T) {
	var gotAnthropicV string
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotAnthropicV = r.Header.Get("anthropic-version")
		w.WriteHeader(200)
		_, _ = w.Write([]byte(`{}`))
	...
	if gotAnthropicV != "2023-06-01" {
		t.Errorf("anthropic-version default = %q, want 2023-06-01", gotAnthropicV)
	}
}
