...
// RequestMetrics encapsulates observability data for a completed Zen gateway request.
...
// LogObservability emits structured and human-readable logs for Zen requests.
// Phase 1 is tokens/latency only: Zen publishes prices as a human-readable
// docs table with no machine-readable endpoint, so cost stays zero/unknown
...
func LogObservability(logger *slog.Logger, m RequestMetrics) {
	...
	msg := fmt.Sprintf("[Zen] %s | tokens: %s in (%s cached, %.1f%% hit), %s out | %.1f TPS | %.2fs",
		m.Model,
